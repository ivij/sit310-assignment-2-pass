...
void processCommand() {
  if (newData == true)
  {
    ...
    if(instruction == "TURNL") turn_left(data.toInt());
    if(instruction == "TURNR") turn_right(data.toInt());
    if(instruction == "CONTF") cont_forward(data.toInt());
    if(instruction == "CONTB") cont_backward(data.toInt());
...
void forward(int time)
{
  ...
  analogWrite(motorright,0500); 
  digitalWrite(motorleftdir, LOW);
  analogWrite(motorleft, 0500); 
  delay(time);
  stop();
}
...
void backward(int time)
{
  ...
  analogWrite(motorright,time); 
  digitalWrite(motorleftdir, HIGH);
  analogWrite(motorleft, time);
  delay(time);
  stop();
}
